...
with st.sidebar:
    choose = option_menu("App Gallery", ["About", "Today\'s Korea Stock Market", "Predict Korea Stocks", 'Caution'],
                         icons=['house','graph-up-arrow','hypnotize', 'exclamation-diamond-fill'],
...
if choose == "About":
    ...
    with st.spinner('Wait for it...'):
        if Name in Code_name_list:
            ...
            col3.metric("전일 대비 가격", "%d원" %(df['Close'].diff().tail(1)[0]), "%.2f%%" %(df['Close'].pct_change().tail(1)[0] * 100))
    ...
    if Name in Code_name_list:
        code_num = Stockcode.at[Name, 'code']
        data = pdr.get_data_yahoo(code_num)
